...
# catch.dat <- read.csv("../boot/data/Herring_TAC_catches_by_area.csv", sep = ";")
...
```{r Selection commercial per fleet, eval = F}
...
tmp$grp <- cut(tmp$year,breaks=(4), include.lowest=T, right=T)
tmp$fleet <- fleets[i]
faa <- rbind(faa,tmp)
}
...
```{r Estimated cohort, fig.height=7, eval = F}
ggAgeDistStk2(fit)
```
