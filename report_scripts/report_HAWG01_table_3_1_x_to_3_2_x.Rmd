...
data_321 <-
  aggregate(
    cbind((round(caton_t, digits = 1))) ~ year + area + quarter + ctry + noSamples +
      noLength + noAge + fleet,
    data = canumF1,
    FUN = sum
  )
data_321 <- rename(data_321, "SOP_1000t" = V1, "noSample" = noSamples)
data_321 <-
  aggregate(
...
data_321 <- arrange(data_321, year, area, fleet, ctry, quarter)
...
t3 <- flextable(data_321_done) %>%
  ...
  set_caption(
    caption = paste0(
      "Table 3.2.1 WESTERN BALTIC HERRING. Both WBSS and NSAS. Samples of commercial catch available to the Working Group by area, quarter, country and fleet for ",
      year_data
    ),
    style = "Table Caption"
  ) %>%
...
dat_322 <- rename(dat_322, fleet = subFleet, sampling = comment2)
...
t4 <- flextable(dat_322) %>%
  ...
  set_caption(
    caption = paste0(
      "Table 3.2.2 WESTERN BALTIC HERRING. Both WBSS and NSAS. Samples of commercial catch by quarter, fleet and area for ",
      year_data,
