...
mis <- read.csv(paste("../boot/data/imputations/", "C4_her2024_missing_bio_", year_0, "_filled.csv", sep = ""), sep = ";")
...
# Version `r paste0(lubridate::today(), "-v3")`
...
```{r}
samp_all <- distinct(canum_3, ctry, year, quarter, area, subFleet, catch_t, noSample, noLength, noAge)
datatable(subset(samp_all, catch_t > 0 & year == year_0), filter = 'top', options = list(
...
datatable(samp_1, filter = 'top', options = list(
  pageLength = 25, autoWidth = F
),
class = 'cell-border stripe')
```
...
### Knowlegde & imputations, results

```{r}
datatable(subset(mis, catch_t > 0 & subFleet == "passive" & area %in% c("27.3.a.20", "27.3.a.21")), filter = 'top', options = list(
  pageLength = 25, autoWidth = F
),
class = 'cell-border stripe')
```
...
```{r}
datatable(subset(mis, catch_t > 0 & subFleet == "purse seine" & area %in% c("27.3.a.20", "27.3.a.21")), filter = 'top', options = list(
  pageLength = 25, autoWidth = F
),
class = 'cell-border stripe')
```
...
```{r}
datatable(subset(mis, catch_t > 0 & subFleet == "trawl >= 32mm" & area %in% c("27.3.a.20", "27.3.a.21")), filter = 'top', options = list(
  pageLength = 25, autoWidth = F
),
class = 'cell-border stripe')
```
...
# Lan & samp, 27.3.bcd, p, cont.

## Row {data-height="100"}

### Knowlegde & imputations

27.3.b.23: Use Swedish samples on Danish landings quarter per quarter?

27.3.c.22: Use German samples on Danish. Q4 goes to Q3?

27.3.d.24: Q2, German samples on unsampled. Q4 cover all unsampled in Q3 & Q4?
...
```{r}
datatable(subset(mis, catch_t > 0 & subFleet == "passive" & area %in% c("27.3.b.23", "27.3.c.22", "27.3.d.24")), filter = 'top', options = list(
  pageLength = 25, autoWidth = F
),
class = 'cell-border stripe')
```
...
```{r}
datatable(subset(mis, catch_t > 0 & subFleet == "active >= 32mm" & area %in% c("27.3.c.22", "27.3.d.24")), filter = 'top', options = list(
  pageLength = 25, autoWidth = F
),
class = 'cell-border stripe')
```
...
```{r}
datatable(subset(mis, catch_t > 0 & subFleet == "active < 32mm" & area %in% c("27.3.b.23", "27.3.c.22", "27.3.d.24")), filter = 'top', options = list(
  pageLength = 25, autoWidth = F
),
class = 'cell-border stripe')
```
